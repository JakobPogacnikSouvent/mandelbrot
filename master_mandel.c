...
int main(int argc, char *argv[]) {
  ...
  int p_h = h/p;
  ima = (unsigned char *)malloc( w*p_h*sizeof(unsigned char));
  ...
  double p_ymin = ymin + (ymax - ymin)/p * rank;
  ...
  unsigned char *mandel;
  
  if (rank == 0) {
    mandel = (unsigned char *)malloc(w*h*sizeof(unsigned char));
  }
  
  MPI_Gather(ima, w*p_h, MPI_UNSIGNED_CHAR, mandel, w*p_h, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
